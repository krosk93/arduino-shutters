...
Shutters::Shutters(float delay_total_up, float delay_total_down, void (*upCallback)(void), void (*downCallback)(void), void (*haltCallback)(void), unsigned char eeprom_offset)
: _moving(false)
, _reached(false)
...
, _delayTotalUp(delay_total_up)
, _delayTotalDown(delay_total_down)
, _upCallback(upCallback)
, _downCallback(downCallback)
, _haltCallback(haltCallback)
{
  this->_delayOneLevelUp = delay_total_up / LEVELS;
  this->_delayOneLevelDown = delay_total_down / LEVELS;
}
...
bool Shutters::begin() {
  if(!savedIsLastLevelKnown()) {
    ...
    up();
    delay((this->_delayTotalUp + this->_delayOneLevelUp * CALIBRATION_LEVELS) * 1000);
    halt();
...
void Shutters::loop() {
  ...
  if (this->_moving) {
    ...
    if ((this->_direction == DIRECTION_DOWN && now - this->_timeLastLevel >= this->_delayOneLevelDown * 1000) || (this->_direction == DIRECTION_UP && now - this->_timeLastLevel >= this->_delayOneLevelUp * 1000)) {
