...
private:
  ...
  float _delayTotalUp;
  float _delayTotalDown;
  float _delayOneLevelUp;
  float _delayOneLevelDown;
...
public:
  Shutters(float delay_total_up, float delay_total_down, void (*upCallback)(void), void (*downCallback)(void), void (*haltCallback)(void), unsigned char eeprom_offset = 0);
  bool begin();
  void loop();
